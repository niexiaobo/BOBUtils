...
- (void)executeHttpRequest:(NSString *)url
               requestName:(NSString *)requestName
                    ...
                    params:(id)params
                     block:(void (^)(NTHttpRequest *networkRequest,
                                     NSError *error))block {
  NSLog(@"url:%@", url);
  ...
  if (methodType == NTRequestMethodGET) {
    ...
    [manager POST:strurl
        ...
        constructingBodyWithBlock:^(id<AFMultipartFormData> _Nonnull formData) {
          ...
          //            URLForResource:@"test.jpg" withExtension:nil];
          // 要上传保存在服务器中的名称
        ...
        success:^(NSURLSessionDataTask *_Nonnull task,
          ...
          NSString *str =
              [[NSString alloc] initWithData:responseObject encoding:enc];
          if (!str) {
            str = [[NSString alloc] initWithData:responseObject
...
- (void)postImgWithParam:(NSDictionary *)dic
        ...
        if (arr) {
          if (arr) {
            for (int i = 0; i < arr.count; i++) {
              ...
              //                        eachImg = [self imageWithImage:eachImg
              //                        scaledToSize:imagesize];
              //                        NSData *eachImgData =
              //                        UIImageJPEGRepresentation(eachImg, 0.5);
              //                        [formData
              //                        appendPartWithFileData:eachImgData
              //                        name:[NSString
              //                        stringWithFormat:@"file%d.jpg", i+1]
              //                        fileName:[NSString
              //                        stringWithFormat:@"images"]
              //                        mimeType:@"image/jpeg"];
              //                    }else{
              ...
              [formData
                  appendPartWithFileData:eachImgData
                                    name:[NSString
                                             stringWithFormat:@"file%d.jpg",
                                                              i + 1]
                                fileName:[NSString stringWithFormat:@"images"]
                                mimeType:@"image/jpeg"];
              // }
            }
          }
          if (voiceData) {
            //语音
            [formData
                appendPartWithFileData:voiceData
                                  name:@"Audio.wav"
                              fileName:[NSString stringWithFormat:@"Audio"]
                              mimeType:@"audio/wav"];
          }
        }
